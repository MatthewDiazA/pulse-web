...
export default function JoinCrew() {
  ...
  useEffect(() => {
    const join = async () => {
      ...
      const { data: crew } = await supabase.from('crews').select('name').eq('id', crewId).single()
  ...
  return (
    <>
      <style>{`*{margin:0;padding:0;box-sizing:border-box;}body{background:${COLORS.bg};font-family:'DM Sans',sans-serif;color:#f0f0f0;}`}</style>
