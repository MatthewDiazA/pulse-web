...
export default function EventPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleCheckout = async () => {
    ...
    if (tier.price === 0) {
      ...
      const res = await fetch('/api/checkout/free', {
        ...
        body: JSON.stringify({
          event_id: id, tier_id: tier.id, tier_name: tier.name,
          event_title: event.title, quantity,
    ...
    const res = await fetch('/api/checkout', {
      ...
      body: JSON.stringify({
        event_id: id, tier_id: tier.id, tier_name: tier.name,
        price: total, quantity, event_title: event.title,
      }),
    })
    const data = await res.json()
  ...
  if (!event) return (
    <div style={{background:'#0a0a0b', minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', color:'#888', fontFamily:'DM Sans,sans-serif'}}>
      Event not found. <span style={{color:'#ED1465', cursor:'pointer', marginLeft:'8px'}} onClick={() => window.location.href='/'}>← Back to events</span>
    </div>
  )
    ...
    <>
      <style>{`
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        nav { padding:14px 0; background:rgba(10,10,11,0.95); position:sticky; top:0; z-index:100; backdrop-filter:blur(16px); -webkit-backdrop-filter:blur(16px); position:relative; }
        ...
        .hero-tag { position:absolute; top:20px; left:20px; font-size:11px; padding:4px 12px; border-radius:100px; background:rgba(237,20,101,0.18); color:#ED1465; border:0.5px solid rgba(237,20,101,0.3); text-transform:uppercase; letter-spacing:0.5px; font-family:'DM Sans',sans-serif; }
        .hero-age { position:absolute; top:20px; right:20px; font-size:11px; padding:4px 12px; border-radius:100px; background:rgba(255,255,255,0.1); color:#f0f0f0; border:0.5px solid rgba(255,255,255,0.14); font-family:'DM Sans',sans-serif; }
        ...
        .event-date { font-size:13px; color:#ED1465; letter-spacing:0.8px; text-transform:uppercase; font-weight:500; margin-bottom:12px; font-family:'DM Sans',sans-serif; }
        ...
        .host-avatar { width:32px; height:32px; border-radius:50%; background:rgba(237,20,101,0.2); display:flex; align-items:center; justify-content:center; font-size:12px; font-weight:500; color:#ED1465; font-family:'DM Sans',sans-serif; }
        .host-name { font-size:13px; color:#888; font-family:'DM Sans',sans-serif; }
        .section-title { font-size:11px; color:#555; letter-spacing:0.8px; text-transform:uppercase; margin-bottom:14px; font-family:'DM Sans',sans-serif; }
        ...
        .tier.selected { border-color:#ED1465; background:rgba(237,20,101,0.04); }
        ...
        .tier-price { font-family:'Barlow Condensed',sans-serif; font-size:22px; color:#ED1465; font-weight:900; }
        ...
        .buy-btn { width:100%; background:#ED1465; color:#fff; border:none; border-radius:100px; padding:14px; font-size:15px; font-weight:700; cursor:pointer; font-family:'Nunito',sans-serif; box-shadow:0 0 16px rgba(237,20,101,0.25); transition:all 0.15s; }
        .buy-btn:active { transform:scale(0.98); }
        .buy-btn:disabled { background:#222; color:#555; cursor:not-allowed; box-shadow:none; }
        ...
        {event.cover_image_url
          ...
          : (
            <svg width="100%" height="100%" viewBox="0 0 1100 400" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice">
              ...
              <circle cx="550" cy="200" r="150" fill="none" stroke="#ED1465" strokeWidth="0.5" opacity="0.2"/>
              <circle cx="550" cy="200" r="220" fill="none" stroke="#ED1465" strokeWidth="0.5" opacity="0.1"/>
              <circle cx="550" cy="200" r="50" fill="rgba(237,20,101,0.06)"/>
              <text x="550" y="215" textAnchor="middle" fontFamily="serif" fontSize="64" fill="#ED1465" opacity="0.8">✦</text>
            </svg>
          )
        }
