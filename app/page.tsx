    ...
    return (
      <div style={{display:'flex', gap:'8px', alignItems:'center'}}>
        ...
        <a href="/account" style={{
          background:'transparent', color:'#ffaa33', fontSize:'13px', fontWeight:600,
          padding:'8px 18px', borderRadius:'6px', border:'0.5px solid rgba(255,170,51,0.3)',
...
function MobileNavBtn() {
  ...
  if (user) {
    return (
      <div style={{display:'flex', gap:'8px', alignItems:'center'}}>
        <a href="/host/create" style={{
          background:'#ffaa33', color:'#000', fontSize:'12px', fontWeight:700,
          padding:'7px 14px', borderRadius:'100px', textDecoration:'none',
          fontFamily:'Nunito,sans-serif', boxShadow:'0 0 14px rgba(255,170,51,0.3)',
        ...
        <a href="/account" style={{
          background:'transparent', color:'#ffaa33', fontSize:'12px', fontWeight:600,
          padding:'7px 14px', borderRadius:'6px', border:'0.5px solid rgba(255,170,51,0.3)',
          textDecoration:'none', fontFamily:'Nunito,sans-serif',
          display:'inline-flex', alignItems:'center', gap:'4px',
        }}>
...
const placeholderAccent: Record<string, string> = {
  nightlife: '#ffaa33', concert: '#ff6600', festival: '#ffc850', other: '#ff8800',
}
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // sweeping stage rays from top center
      const numRays = 10
      for (let i = 0; i < numRays; i++) {
        const angle = -Math.PI * 0.85 + (i / (numRays - 1)) * Math.PI * 0.7
        const sway = Math.sin(t * 0.5 + i * 0.7) * 0.06
        const finalAngle = angle + sway
        const len = H * 1.2
        const ex = cx + Math.cos(finalAngle) * len
        const ey = Math.sin(finalAngle) * len
      ...
      // grid of warm amber dots — Saint Pablo light rig
      const cols = 14
      const rows = 9
      ...
      // amber fog haze — center glow like fog machine under the platform
      const fogGrad = ctx.createRadialGradient(cx, H * 0.52, 0, cx, H * 0.52, W * 0.45)
      fogGrad.addColorStop(0, `rgba(255,120,20,${0.045 + 0.03 * Math.sin(t * 0.7)})`)
      fogGrad.addColorStop(0.5, `rgba(255,80,0,${0.02 + 0.01 * Math.sin(t * 0.5)})`)
      fogGrad.addColorStop(1, 'rgba(255,60,0,0)')
      ctx.fillStyle = fogGrad
      ctx.fillRect(0, 0, W, H)

      // subtle crowd haze at bottom
      const crowdGrad = ctx.createLinearGradient(0, H * 0.7, 0, H)
      crowdGrad.addColorStop(0, 'rgba(0,0,0,0)')
      crowdGrad.addColorStop(1, `rgba(255,80,10,${0.04 + 0.02 * Math.sin(t * 0.3)})`)
    ...
    <>
      ...
      <style>{`
        ...
        * { margin:0; padding:0; box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
        ...
        nav { padding:14px 0; background:rgba(0,0,0,0.9); position:sticky; top:0; z-index:100; backdrop-filter:blur(20px); -webkit-backdrop-filter:blur(20px); position:relative; }
        ...
        .logo { font-family:'Nunito',sans-serif; font-size:28px; font-weight:900; letter-spacing:-0.5px; color:#ffaa33; cursor:pointer; line-height:1; text-transform:lowercase; filter:drop-shadow(0 0 10px rgba(255,170,51,0.5)); }
        ...
        .hero-fade { position:absolute; bottom:0; left:0; right:0; height:180px; background:linear-gradient(to bottom,transparent,#000); pointer-events:none; z-index:1; }
        ...
        .hero-word.accent { color:#ffaa33; text-shadow:0 0 60px rgba(255,170,51,0.5), 0 0 120px rgba(255,100,0,0.25); white-space:nowrap; }
        .hero-word.show { transform:translateY(0); }
        .hero-sub { position:relative; z-index:2; margin-top:16px; padding:0 20px; font-size:14px; color:#443; font-weight:300; font-family:'DM Sans',sans-serif; line-height:1.6; opacity:0; transition:opacity 0.8s ease 0.6s; }
        .hero-sub.show { opacity:1; }
        .hero-sub a { color:#ffaa33; text-decoration:none; font-weight:500; display:inline-flex; align-items:center; gap:4px; }
        ...
        .ticker-item { font-family:'Barlow Condensed',sans-serif; font-size:12px; font-weight:700; letter-spacing:2px; color:#ffaa33; opacity:0.3; white-space:nowrap; text-transform:uppercase; }
        ...
        .pill { background:rgba(255,255,255,0.04); border:0.5px solid rgba(255,255,255,0.07); border-radius:100px; padding:8px 16px; font-size:13px; color:#443; cursor:pointer; font-family:'DM Sans',sans-serif; transition:all 0.2s; white-space:nowrap; flex-shrink:0; display:inline-flex; align-items:center; gap:6px; }
        .pill:active { transform:scale(0.94); }
        .pill.active { background:rgba(255,170,51,0.1); color:#ffaa33; border-color:rgba(255,170,51,0.3); font-weight:500; }
        ...
        .card { border-radius:14px; cursor:pointer; position:relative; overflow:hidden; opacity:0; transform:translateY(20px); transition:transform 0.25s ease, box-shadow 0.25s ease, opacity 0.4s ease; aspect-ratio:2/3; background:#0d0800; }
        .card.visible { opacity:1; transform:translateY(0); }
        .card:active { transform:scale(0.96) !important; }
        @media(hover:hover){ .card:hover { transform:translateY(-4px) !important; box-shadow:0 20px 60px rgba(0,0,0,0.8), 0 0 30px rgba(255,170,51,0.07); } }
        ...
        .card-price-badge.free { color:#ffaa33; border-color:rgba(255,170,51,0.35); }
        ...
        .tag-nightlife { background:rgba(255,170,51,0.12); color:#ffaa33; border:0.5px solid rgba(255,170,51,0.25); }
        .tag-concert { background:rgba(255,102,0,0.12); color:#ff6600; border:0.5px solid rgba(255,102,0,0.25); }
        .tag-festival { background:rgba(255,200,80,0.1); color:#ffc850; border:0.5px solid rgba(255,200,80,0.2); }
        ...
        .empty a { color:#ffaa33; text-decoration:none; display:inline-flex; align-items:center; gap:4px; }
            ...
            {loading ? (
              ...
              <div className="grid" ref={gridRef}>
                {filtered.map((event, index) => {
                  ...
                  const accent = placeholderAccent[cat] ?? '#ffaa33'
                    ...
                    <div key={event.id} className="card" data-index={index} onClick={() => window.location.href=`/events/${event.id}`}>
                      {event.cover_image_url ? (
                        ...
                        <svg width="100%" height="100%" viewBox="0 0 200 300" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice" style={{position:'absolute',inset:0}}>
                          ...
                          <rect width="200" height="300" fill="#0d0800"/>
                          ...
                          {[0,1,2,3,4,5,6].map((_,i) => (
                            <circle key={i} cx={20+i*27} cy={30+Math.sin(i)*15} r="2" fill={accent} opacity={0.2+i*0.05}/>
                          ))}
                          {[0,1,2,3,4,5,6].map((_,i) => (
                            <circle key={i} cx={25+i*26} cy={55+Math.cos(i)*10} r="2" fill={accent} opacity={0.15+i*0.04}/>
                          ))}
                        </svg>
                      )}
