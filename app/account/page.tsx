...
function TicketModal({ ticket, onClose }: { ticket: any, onClose: () => void }) {
  ...
  const date = ticket.event?.starts_at ? new Date(ticket.event.starts_at).toLocaleDateString('en-US', { weekday:'short', month:'short', day:'numeric' }) : 'Date TBD'
      ...
      <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.92)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:'20px',backdropFilter:'blur(16px)',animation:'backdropIn 0.3s ease forwards'}} onClick={onClose}>
        {phase === 'entry' && (
          <div className="slide-up" style={{width:'320px'}} onClick={e => e.stopPropagation()}>
            <div style={{background:'linear-gradient(135deg,#1a0f00 0%,#0d0800 100%)',border:'0.5px solid rgba(255,255,255,0.08)',borderRadius:'20px',overflow:'hidden',boxShadow:'0 40px 80px rgba(0,0,0,0.8)'}}>
              ...
              <div style={{padding:'28px',textAlign:'center'}}>
                <div style={{fontSize:'11px',letterSpacing:'3px',color:'#443',textTransform:'uppercase',marginBottom:'12px',fontFamily:'DM Sans,sans-serif'}}>Your ticket</div>
                ...
                <div style={{fontSize:'13px',color:'#665',fontFamily:'DM Sans,sans-serif'}}>{ticket.tier?.name} · {date}</div>
                <div style={{marginTop:'20px',width:'48px',height:'48px',borderRadius:'50%',background:'rgba(255,170,51,0.1)',border:'1px solid rgba(255,170,51,0.3)',display:'flex',alignItems:'center',justifyContent:'center',margin:'20px auto 0'}}>
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#ffaa33" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M2 9a1 1 0 0 1 1-1h18a1 1 0 0 1 1 1v2a2 2 0 0 0 0 4v2a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1v-2a2 2 0 0 0 0-4V9z"/><line x1="9" y1="8" x2="9" y2="16" strokeDasharray="2 2"/></svg>
                </div>
              </div>
              <div style={{borderTop:'2px dashed rgba(255,255,255,0.08)',margin:'0 20px'}}/>
              <div style={{padding:'20px',textAlign:'center'}}>
                <div style={{fontSize:'12px',color:'#443',letterSpacing:'1px',textTransform:'uppercase',fontFamily:'DM Sans,sans-serif'}}>{ticket.event?.venue_name ?? 'Venue TBD'}</div>
              </div>
            </div>
          </div>
        )}
        {phase === 'tear' && (
          <div style={{width:'320px',position:'relative'}} onClick={e => e.stopPropagation()}>
            <div className="tear-top" style={{background:'linear-gradient(135deg,#1a0f00 0%,#0d0800 100%)',border:'0.5px solid rgba(255,255,255,0.08)',borderRadius:'20px 20px 0 0',borderBottom:'2px dashed rgba(255,255,255,0.1)',padding:'28px 28px 20px',textAlign:'center'}}>
              <div style={{height:'3px',background:'linear-gradient(90deg,#ff6600,#ffaa33)',borderRadius:'2px',marginBottom:'20px'}}/>
              <div style={{fontSize:'11px',letterSpacing:'3px',color:'#443',textTransform:'uppercase',marginBottom:'8px',fontFamily:'DM Sans,sans-serif'}}>Your ticket</div>
            ...
            <div className="tear-bottom" style={{background:'linear-gradient(135deg,#0d0800 0%,#1a0f00 100%)',border:'0.5px solid rgba(255,255,255,0.08)',borderRadius:'0 0 20px 20px',borderTop:'2px dashed rgba(255,255,255,0.1)',padding:'20px 28px 28px',textAlign:'center'}}>
              <div style={{fontSize:'13px',color:'#665',fontFamily:'DM Sans,sans-serif',marginBottom:'12px'}}>{ticket.tier?.name} · {date}</div>
            </div>
          </div>
        )}
        {phase === 'open' && (
          <div className="reveal-in glow-ring" style={{background:'linear-gradient(135deg,#1a0f00 0%,#0d0800 100%)',border:'1px solid rgba(255,170,51,0.35)',borderRadius:'24px',padding:'32px',maxWidth:'360px',width:'100%',textAlign:'center',position:'relative',overflow:'hidden'}} onClick={e => e.stopPropagation()}>
            <div className="shimmer-line" style={{position:'absolute',top:0,left:0,right:0,height:'100%',pointerEvents:'none',borderRadius:'24px'}}/>
            <button onClick={onClose} style={{position:'absolute',top:'16px',right:'16px',background:'rgba(255,255,255,0.05)',border:'0.5px solid rgba(255,255,255,0.08)',color:'#443',width:'32px',height:'32px',borderRadius:'50%',cursor:'pointer',fontSize:'18px',display:'flex',alignItems:'center',justifyContent:'center'}}>×</button>
            ...
            <div style={{fontSize:'13px',color:'#665',marginBottom:'4px',fontFamily:'DM Sans,sans-serif'}}>{ticket.tier?.name} · {date}</div>
            <div style={{fontSize:'12px',color:'#443',marginBottom:'24px',fontFamily:'DM Sans,sans-serif'}}>{ticket.event?.venue_name ?? ''}</div>
            <div className="qr-reveal" style={{background:'#ffffff',borderRadius:'16px',padding:'16px',display:'inline-block',cursor:'pointer',transition:'transform 0.15s',transform:`scale(${scale})`,boxShadow:'0 0 40px rgba(255,170,51,0.15)'}} onTouchStart={() => setScale(1.08)} onTouchEnd={() => setScale(1)} onMouseDown={() => setScale(1.08)} onMouseUp={() => setScale(1)}>
              {dataUrl ? <img src={dataUrl} alt="QR Code" style={{width:'200px',height:'200px',display:'block'}}/> : <div style={{width:'200px',height:'200px',display:'flex',alignItems:'center',justifyContent:'center',color:'#888',fontSize:'13px'}}>Generating...</div>}
            </div>
            <div style={{marginTop:'20px',fontSize:'11px',color:'#443',letterSpacing:'1.5px',textTransform:'uppercase',fontFamily:'DM Sans,sans-serif'}}>Show at the door · Tap to enlarge</div>
          </div>
        )}
      </div>
    </>
...
export default function AccountPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      setUser(user)
      const { data } = await supabase.from('tickets').select('*, event:events(title,starts_at,venue_name,cover_image_url), tier:ticket_tiers(name,price)').eq('user_id', user.id).order('created_at', { ascending: false })
      setTickets(data ?? [])
      setLoading(false)
    }
    fetchData()
    ...
    <>
      ...
      <style>{`
        ...
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        .nav-btn{font-size:13px;color:#554;background:none;border:none;cursor:pointer;font-family:'DM Sans',sans-serif;text-decoration:none;transition:color 0.15s;display:inline-flex;align-items:center;gap:5px;}
        .nav-btn:hover{color:#f0f0f0;}
        .create-btn{background:#ffaa33;color:#000;font-size:13px;font-weight:700;padding:8px 16px;border-radius:100px;border:none;cursor:pointer;font-family:'Nunito',sans-serif;box-shadow:0 0 14px rgba(255,170,51,0.3);text-decoration:none;display:inline-flex;align-items:center;gap:6px;}
        ...
        .success-banner{background:rgba(255,170,51,0.07);border:0.5px solid rgba(255,170,51,0.25);border-radius:10px;padding:16px 20px;margin-bottom:32px;display:flex;align-items:center;gap:12px;animation:slideUp 0.5s cubic-bezier(0.34,1.2,0.64,1) forwards;}
      ...
      <nav>
        ...
        <div className="nav-right">
          <a href="/" className="nav-btn"><i className="ti ti-arrow-left" style={{fontSize:'13px'}}/>Discover</a>
          <a href="/host" className="nav-btn" style={{background:'rgba(255,170,51,0.08)',color:'#ffaa33',padding:'7px 14px',borderRadius:'6px',border:'0.5px solid rgba(255,170,51,0.25)'}}>Host dashboard</a>
          <a href="/host/create" className="create-btn"><i className="ti ti-plus" style={{fontSize:'13px'}}/>create event</a>
        </div>
      </nav>
      ...
      <div className="wrap">
        {orderSuccess && (
          <div className="success-banner" style={{marginTop:'32px'}}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#ffaa33" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M2 9a1 1 0 0 1 1-1h18a1 1 0 0 1 1 1v2a2 2 0 0 0 0 4v2a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1v-2a2 2 0 0 0 0-4V9z"/><line x1="9" y1="8" x2="9" y2="16" strokeDasharray="2 2"/></svg>
            <div><div className="success-text">Payment successful — you're on the list.</div><div className="success-sub">Your tickets have been sent to your email.</div></div>
          </div>
        )}
        ...
        <div className="quick-actions">
          <a href="/" className="action-card"><div className="action-title">Find events</div><div className="action-sub">Browse parties & concerts near you</div></a>
          <a href="/host/create" className="action-card accent"><div className="action-title">✦ Host an event</div><div className="action-sub">Create and sell tickets in minutes</div></a>
        </div>
        {user && (
          <div className="user-card">
            <div className="user-avatar">{initials}</div>
            <div><div className="user-name">{user.user_metadata?.full_name ?? 'No name set'}</div><div className="user-email">{user.email}</div></div>
            <button className="signout-btn" onClick={async () => { const s = createClient(); await s.auth.signOut(); window.location.href = '/' }}>Sign out</button>
          </div>
        )}
