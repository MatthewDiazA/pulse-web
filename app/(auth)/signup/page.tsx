...
export default function SignupPage() {
  ...
  const handleSignup = async () => {
    ...
    const { error } = await supabase.auth.signUp({
      email, password,
      options: { data: { full_name: name }, emailRedirectTo: 'https://pulse-one-hazel.vercel.app/auth/callback' }
    })
      ...
      <>
        ...
        <style>{`
          ...
          *{margin:0;padding:0;box-sizing:border-box;}
          ...
          .sub{font-size:14px;color:#665;line-height:1.7;margin-bottom:8px;font-family:'DM Sans',sans-serif;}
          .email-highlight{color:#ffaa33;font-weight:500;}
          .hint{font-size:12px;color:#332;margin-top:24px;font-family:'DM Sans',sans-serif;}
          .back-btn{margin-top:20px;display:inline-block;font-size:13px;color:#554;font-family:'DM Sans',sans-serif;cursor:pointer;transition:color 0.15s;}
        ...
        <div className="page">
          ...
          <div className="card">
            ...
            <div className="icon-wrap"><i className="ti ti-mail" style={{fontSize:'32px', color:'#ffaa33'}}/></div>
    ...
    <>
      ...
      <div className="page">
        ...
        <div className="card">
          ...
          <div className="divider"><div className="divider-line"/><div className="divider-text">sign up with email</div><div className="divider-line"/></div>
          <div className="field">
          ...
          <div className="field">
            ...
            <input className="input" type="password" placeholder="At least 8 characters" value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSignup()}/>
          </div>
          {error && <div className="error">{error}</div>}
          <button className="submit-btn" disabled={loading} onClick={handleSignup}>{loading ? 'Creating account...' : "Let's go →"}</button>
